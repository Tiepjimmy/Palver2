...
class File extends Command
{
     ...
     * @throws Exception
     */
    public function createModel(string $sdk, string $module, string $model, string $table)
    {
        $moduleName = Str::ucfirst($module);
        $modelName = Str::ucfirst($model);

        $modelPath = base_path("vendor/palee/$sdk/src/Modules/$moduleName/Models");
        $namespace = Str::ucfirst(Str::camel($sdk)) . "\\Modules\\$moduleName\\Models";
        ...
        $fillable = implode(",\n" . str_repeat(' ', 8), array_map(function ($field) {
            return '\'' . $field . '\'';
        }, array_diff($schema, array_merge($this->getExcerptColumns(), ['created_at', 'updated_at', 'deleted_at'])))) . ',';
...
//                    '~\#\s+\[auto-gen-attribute\](.*?)(.|[\r\n\t])*?\s+\[\/auto-gen-attribute\]~' => function($match) use($fillable) {
//                        return "# [auto-gen-attribute]\n\t\t$fillable\n\t\t# [/auto-gen-attribute]";
//                    }
                ],
                $content
            );
     ...
     * @throws Exception
     */
    public function createRepository(string $sdk, string $module, string $repository)
    {
        $moduleName = Str::ucfirst($module);
        $repositoryName = Str::ucfirst($repository) . 'Repository';
        $repositoryInterfaceName = 'I' . $repositoryName;
        $sdkName = Str::ucfirst(Str::camel($sdk));

        $repositoryInterfacePath = base_path("vendor/palee/$sdk/src/Modules/$moduleName/Repositories/Contracts");
        $repositoryPath = base_path("vendor/palee/$sdk/src/Modules/$moduleName/Repositories/Eloquent");
        $repositoryInterfaceNamespace = $sdkName . "\\Modules\\$moduleName\\Repositories\\Contracts";
        $repositoryNamespace = $sdkName . "\\Modules\\$moduleName\\Repositories\\Eloquent";
        $repositoryInterface = $repositoryInterfaceNamespace  . '\\' . $repositoryInterfaceName;
        $repositoryClass = $repositoryNamespace . '\\' . $repositoryName;

        if (! file_exists($repositoryPath)) {
            mkdir($repositoryPath, 0775, true);
        }
        ...
        $servicesRegisterPath = base_path("vendor/palee/$sdk/src/RepositoryServiceProvider.php");
        $servicesRegisterClass = Str::ucfirst(Str::camel($sdk)) . "\\RepositoryServiceProvider";

        if (! class_exists($servicesRegisterClass)) {
            throw new Exception('RepositoryServiceProvider class not found');
        }

        $services = (new $servicesRegisterClass(app()))->getServicesList();

        if (array_key_exists($repositoryInterface, $services)) {
            unset($services[$repositoryInterface]);
        }

        $services[$repositoryInterface] = $repositoryClass;
 ...
 */
class $repositoryName extends AbstractEloquentRepository implements $repositoryInterfaceName
{
    /**
     * Map model
     * @return void
...
interface $repositoryInterfaceName extends AbstractEloquentInterface
{
...
CONTENT;
        ...
        $file = fopen($repositoryPath .'/'. $repositoryName . '.php', 'w');
        fwrite($file, $repositoryContent);
        fclose($file);

        // Create repository file
        $file = fopen($repositoryInterfacePath .'/'. $repositoryInterfaceName . '.php', 'w');
        fwrite($file, $repositoryInterfaceContent);
        fclose($file);

        $servicesContent = [];
        foreach ($services as $key => $value) {
            $servicesContent[] = "'$key' => '$value'";
        }
        $servicesContent = implode(",\n" . str_repeat(' ', 8), $servicesContent) . ',';

        $servicesRegisterContent = file_get_contents($servicesRegisterPath);
        $servicesRegisterContent = preg_replace_callback_array(
            [
                '~\#\s+\[auto-gen-service\](.*?)(.|[\r\n\t])*?\#\s+\[\/auto-gen-service\]~' => function() use ($servicesContent) {
                    return "# [auto-gen-service]\n" . str_repeat(' ', 8) . "$servicesContent\n" . str_repeat(' ', 8) . "# [/auto-gen-service]";
                }
            ],
            $servicesRegisterContent
        );

        $file = fopen($servicesRegisterPath, 'w');
        fwrite($file, rtrim($servicesRegisterContent));
        fclose($file);
    ...
    public function createRequest(string $module, string $request, string $table)
    {
        ...
        $requestPath = app_path("Modules/$moduleName/Requests");
 ...
 * [/auto-gen-property]
 */
class $requestName extends AbstractRequest
{
    /**
     * @return string[]
     */
...
//            $ret = 'Request "\\' . $namespace . '\\' . $requestName . '" was updated!';
    ...
    public function createRequestStore(string $module, string $request, string $table)
    {
        ...
        if (! file_exists(app_path("Modules/$moduleName/Requests/" . $requestName . '.php'))) {
...
//            return 'Request "\\' . $namespace . '\\' . $requestStoreName . '" was updated!';
    ...
    public function createRequestUpdate(string $module, string $request, string $table)
    {
        ...
        if (! file_exists(app_path("Modules/$moduleName/Requests/" . $requestStoreName . '.php'))) {
...
//            return 'Request "\\' . $namespace . '\\' . $requestUpdateName . '" was updated!';
    ...
    public function createResource(string $module, string $resource, string $table)
    {
        ...
        $resourcePath = app_path("Modules/$moduleName/Resources");
...
//            $ret = 'Resource "\\' . $namespace . '\\' . $resourceName . '" was updated!';
    ...
    /**
     * Get real type of entity column
     * @param $type
     * @return string|null
     */
    protected function getRealType($type)
    {
        if (preg_match('/^(bigint|int|integer|smallint|tinyint)(\(([\d]+)\))?/', $type, $matches)) {
            return 'int';
        }
